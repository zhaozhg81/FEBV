  ...
  #{
    ...
    ##-- Simulation ---------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      #  rebayes <- GVmix(sSq, array(df,G) )$dy
        ...
        ##----------------------------------------------------------------------------------------------------    
        ## L0
        tsse0.sSq[i] <-(mean((sSq0[i]-sigmaSq0)^2,na.rm=TRUE))  
        tsse0.ljs[i] <-(mean((ljs0[i]-sigmaSq0)^2,na.rm=TRUE))  
        tsse0.opt[i] <-(mean((opt0[i]-sigmaSq0)^2,na.rm=TRUE))  
        tsse0.smy[i] <-(mean((smy0[i]-sigmaSq0)^2,na.rm=TRUE))  
        tsse0.vsh[i] <-(mean((vsh0[i]-sigmaSq0)^2,na.rm=TRUE))  
        tsse0.feb[i] <-(mean((feb0[i]-sigmaSq0)^2,na.rm=TRUE))  
        tsse0.fes[i] <-(mean((fes0[i]-sigmaSq0)^2,na.rm=TRUE))  
        tsse0.Feb[i] <-(mean((Feb0[i]-sigmaSq0)^2,na.rm=TRUE))
        tsse0.rebayes[i] <-(mean((rebayes0[i]-sigmaSq0)^2,na.rm=TRUE))  
        
    
        ##L1p
        tssep.sSq[i] <-(mean((sSq0[i]/sigmaSq0-1)^2,na.rm=TRUE))
        tssep.ljs[i] <-(mean((ljs0[i]/sigmaSq0-1)^2,na.rm=TRUE))
        tssep.opt[i] <-(mean((opt0[i]/sigmaSq0-1)^2,na.rm=TRUE))
        tssep.smy[i] <-(mean((smy0[i]/sigmaSq0-1)^2,na.rm=TRUE))
        tssep.vsh[i] <-(mean((vsh0[i]/sigmaSq0-1)^2,na.rm=TRUE))
        tssep.feb[i] <-(mean((feb0[i]/sigmaSq0-1)^2,na.rm=TRUE))
        tssep.fes[i] <-(mean((fes0[i]/sigmaSq0-1)^2,na.rm=TRUE))
        tssep.Feb[i] <-(mean((Feb0[i]/sigmaSq0-1)^2,na.rm=TRUE))
        tssep.rebayes[i] <-(mean((rebayes0[i]/sigmaSq0-1)^2,na.rm=TRUE))
        
        ##L1
        tsse1.sSq[i] <-(mean((sigmaSq0/sSq0[i]-1)^2,na.rm=TRUE))
        tsse1.ljs[i] <-(mean((sigmaSq0/ljs0[i]-1)^2,na.rm=TRUE))
        tsse1.opt[i] <-(mean((sigmaSq0/opt0[i]-1)^2,na.rm=TRUE))
        tsse1.smy[i] <-(mean((sigmaSq0/smy0[i]-1)^2,na.rm=TRUE))
        tsse1.vsh[i] <-(mean((sigmaSq0/vsh0[i]-1)^2,na.rm=TRUE))
        tsse1.feb[i] <-(mean((sigmaSq0/feb0[i]-1)^2,na.rm=TRUE))
        tsse1.fes[i] <-(mean((sigmaSq0/fes0[i]-1)^2,na.rm=TRUE))
        tsse1.Feb[i] <-(mean((sigmaSq0/Feb0[i]-1)^2,na.rm=TRUE))
        tsse1.rebayes[i] <-(mean((sigmaSq0/rebayes0[i]-1)^2,na.rm=TRUE))
    
        ##L2
        tsse2.sSq[i] <-(mean((sSq0[i]/sigmaSq0-log(sSq0[i]/sigmaSq0)-1),na.rm=TRUE))
        tsse2.ljs[i] <-(mean((ljs0[i]/sigmaSq0-log(ljs0[i]/sigmaSq0)-1),na.rm=TRUE))
        tsse2.opt[i] <-(mean((opt0[i]/sigmaSq0-log(opt0[i]/sigmaSq0)-1),na.rm=TRUE))
        tsse2.smy[i] <-(mean((smy0[i]/sigmaSq0-log(smy0[i]/sigmaSq0)-1),na.rm=TRUE))
        tsse2.vsh[i] <-(mean((vsh0[i]/sigmaSq0-log(vsh0[i]/sigmaSq0)-1),na.rm=TRUE))
        tsse2.feb[i] <-(mean((feb0[i]/sigmaSq0-log(feb0[i]/sigmaSq0)-1),na.rm=TRUE))
        tsse2.fes[i] <-(mean((fes0[i]/sigmaSq0-log(fes0[i]/sigmaSq0)-1),na.rm=TRUE))
        tsse2.Feb[i] <-(mean((Feb0[i]/sigmaSq0-log(Feb0[i]/sigmaSq0)-1),na.rm=TRUE))
        tsse2.rebayes[i] <-(mean((rebayes0[i]/sigmaSq0-log(rebayes0[i]/sigmaSq0)-1),na.rm=TRUE))
