...
plot( Result.unknown[,1],Result.unknown[,2],xlab="M",ylab="Coverage Prob", ylim=c(0,1), xlim=c(0,1),col="white",main=bquote(paste("[Coverage Prob.]   ",.(PRIOR), "    a=",.(a),",  b=",.(b),",  df=",.(df) )), cex.lab=1.2, cex.axis=1.2, cex.main=1.2)
...
par(xpd=TRUE)
legend("topright",horiz=F, inset = c(-0.3,0.2), legend=c("Error Bar","Bonferroni","ELJS","TW","Smyth","Vash","f-EBV","f-EBVS","F-EBV","SOC"),col=c("black","cyan3","blue","purple","green4","gold4", "brown","orange","red","green"),pch=c(NA,3,15,7,17,16,9,8,19,10), ncol=1, lty=1:1, cex=1)
dev.off()
...
plot( Result.unknown[,1],Result.unknown[,3],xlab="M",ylab="Length Ratio", ylim=c(0,Llim+0.2), xlim=c(0,1),col="white",main=bquote(paste("[Length Ratio]   ",.(PRIOR), "    a=",.(a),",  b=",.(b),",  df=",.(df) )), cex.lab=1.2, cex.axis=1.2, cex.main=1.2)
