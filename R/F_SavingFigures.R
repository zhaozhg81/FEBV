...
# Coverage 
  ...
  #  lines( Result.unknown[,1],Result.unknown[,34], pch=10, type="b", col="green ") # SOC
  legend("bottomright",horiz=F, legend=c("Error Bar","Bonferroni","ELJS","TW","Smyth","Vash","f-EBV","f-EBVS","F-EBV"),col=c("black","cyan3","blue","purple","green4","gold4", "brown","orange","red"),pch=c(NA,3,15,7,17,16,9,8,19), ncol=1, lty=1:1, cex=1.3)
  dev.off()
  ...
  plot( Result.unknown[,1],Result.unknown[,3],xlab="M",ylab="Length Ratio", ylim=c(0,Llim+0.2), xlim=c(0,1),col="white",main=bquote(paste("[Length]   ",.(PRIOR), "    a=",.(a),",  b=",.(b),",  df=",.(df) )), cex.lab=1.3, cex.axis=1.3, cex.main=1.5)
  lines( Result.unknown[,1],Result.unknown[,3]/Result.unknown[,3] , pch=3 , type="b", col="cyan3" ) # Bonferroni
