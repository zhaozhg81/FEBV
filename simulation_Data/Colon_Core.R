...
if(PLOT==FALSE)
  {
    ...
    ## Colon Data
    colondata <- read.table("./Textdata/colon_x.txt", quote="\"", comment.char="")
    
    G       <- nrow(colondata)   # Dimension
    nobs    <- ncol(colondata)
    ...
    for(i in  1:numSim){
      ...
      colon1  <- t(colondata[,rand1])          
      colon2  <- t(colondata[,rand2])      
      mean1   <- apply(colon1,2,mean)
      mean2   <- apply(colon2,2,mean)
      s1Sq    <- apply(colon1,2,var)
      s2Sq    <- apply(colon2,2,var)
      sSq     <- (s1Sq + s2Sq)/2
            
      bandwidth <- c(0.22,0.22,0.22)
